...
import {
  getIslamArticleBySlug,
  getAllIslamArticles,
} from "../lib/islamArticles";
...
export default function IslamArticlePage() {
  ...
  const article = slug ? getIslamArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/islam/article/${articleSlug}`;

  const related = getAllIslamArticles()
  ...
  const fallbackRelated = getAllIslamArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-brand-on-accent">
      <Helmet>
        ...
        <meta property="og:image" content={getOgImage("islam", articleSlug)} />
        ...
        <meta name="twitter:image" content={getOgImage("islam", articleSlug)} />
      </Helmet>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/islam/article/${rel.slug}`);
