...
import {
  getNaukaArticleBySlug,
  getAllNaukaArticles,
} from "../lib/naukaArticles";
...
export default function NaukaArticlePage() {
  ...
  const article = slug ? getNaukaArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/nauka/article/${articleSlug}`;

  const related = getAllNaukaArticles()
  ...
  const fallbackRelated = getAllNaukaArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-brand-on-accent">
      <Helmet>
        ...
        <meta property="og:image" content={getOgImage("nauka", articleSlug)} />
        ...
        <meta name="twitter:image" content={getOgImage("nauka", articleSlug)} />
      </Helmet>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/nauka/article/${rel.slug}`);
