...
import {
  getAhmedijeArticleBySlug,
  getAllAhmedijeArticles,
} from "../lib/ahmedijeArticles";
...
export default function AhmedijeArticlePage() {
  ...
  const articleUrl = `${SITE_URL}/categories/ahmedije/article/${articleSlug}`;

  const related = getAllAhmedijeArticles()
  ...
  const fallbackRelated = getAllAhmedijeArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-brand-on-accent">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("ahmedije", articleSlug)}
        />
