...
import {
  getHadisArticleBySlug,
  getAllHadisArticles,
} from "../lib/hadisArticles";
...
export default function HadisArticlePage() {
  ...
  const article = slug ? getHadisArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/hadis/article/${articleSlug}`;

  const related = getAllHadisArticles()
  ...
  const fallbackRelated = getAllHadisArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-brand-on-accent">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("hadis", articleSlug)}
        />
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/hadis/article/${rel.slug}`);
