...
export default function AteizamArticlePage() {
  ...
  const article = slug ? getAteizamArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/ateizam/article/${articleSlug}`;

  const related = getAllAteizamArticles()
  ...
  const fallbackRelated = getAllAteizamArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/ateizam/article/${rel.slug}`);
