...
import {
  getMuhammedArticleBySlug,
  getAllMuhammedArticles,
} from "../lib/muhammedArticles";
...
export default function MuhammedArticlePage() {
  ...
  const article = slug ? getMuhammedArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/muhammed/article/${articleSlug}`;

  const related = getAllMuhammedArticles()
  ...
  const fallbackRelated = getAllMuhammedArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-brand-on-accent">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("muhammed", articleSlug)}
        />
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/muhammed/article/${rel.slug}`);
