...
import {
  getOpovrgavanjeArticleBySlug,
  getAllOpovrgavanjeArticles,
} from "../lib/opovrgavanjeArticles";
...
export default function OpovrgavanjeArticlePage() {
  ...
  const article = slug ? getOpovrgavanjeArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/opovrgavanje/article/${articleSlug}`;

  const related = getAllOpovrgavanjeArticles()
  ...
  const fallbackRelated = getAllOpovrgavanjeArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-white">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("opovrgavanje", articleSlug)}
        />
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        ...
                        onClick={() => {
                          navigate(
                            `/categories/opovrgavanje/article/${rel.slug}`,
                          );
