...
import {
  getOdgovoriArticleBySlug,
  getAllOdgovoriArticles,
} from "../lib/odgovoriArticles";
...
export default function OdgovoriArticlePage() {
  ...
  const article = slug ? getOdgovoriArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/odgovori/article/${articleSlug}`;

  const related = getAllOdgovoriArticles()
  ...
  const fallbackRelated = getAllOdgovoriArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-white">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("odgovori", articleSlug)}
        />
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/odgovori/article/${rel.slug}`);
