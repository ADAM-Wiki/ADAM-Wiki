...
import { gethriscanstvoArticleBySlug, getAllhriscanstvoArticles } from "../lib/hriscanstvoArticles";
...
export default function hriscanstvoArticlePage() {
  ...
  const article = slug ? gethriscanstvoArticleBySlug(slug) : null;
...
useEffect(() => {
  ...
  const words = articleText ? articleText.split(/\s+/).filter(Boolean).length : 0;
  ...
  const { title, description, tags = [], date, slug: articleSlug } = article.meta;
  const metaDescription = truncateDescription(description ?? title);
  const articleUrl = `${SITE_URL}/categories/hriscanstvo/article/${articleSlug}`;

  const related = getAllhriscanstvoArticles()
  ...
  const fallbackRelated = getAllhriscanstvoArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
              ...
              <div className="min-w-0 lg:col-start-2">
                <header className="mb-12">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <h1 className="text-3xl font-serif font-medium text-white">{title}</h1>
                  </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/hriscanstvo/article/${rel.slug}`);
