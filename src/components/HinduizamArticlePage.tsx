...
import {
  getHinduizamArticleBySlug,
  getAllHinduizamArticles,
} from "../lib/hinduizamArticles";
...
export default function HinduizamArticlePage() {
  ...
  const article = slug ? getHinduizamArticleBySlug(slug) : null;
  ...
  const articleUrl = `${SITE_URL}/categories/hinduizam/article/${articleSlug}`;

  const related = getAllHinduizamArticles()
  ...
  const fallbackRelated = getAllHinduizamArticles()
    .filter((item) => item.slug !== articleSlug)
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-brand-bg relative selection:bg-brand-accent selection:text-white">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={getOgImage("hinduizam", articleSlug)}
        />
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {relatedArticles.map((rel) => (
                      <div
                        key={rel.slug}
                        onClick={() => {
                          navigate(`/categories/hinduizam/article/${rel.slug}`);
