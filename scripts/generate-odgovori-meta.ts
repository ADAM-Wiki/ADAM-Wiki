...
type GeneratedOdgovoriMeta = OdgovoriArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/odgovori");
const OUTPUT_FILE = path.resolve("src/lib/generated/odgovoriMeta.ts");
...
function extractMeta(source: string): OdgovoriArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedOdgovoriMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " odgovori articles.",
  );
}
