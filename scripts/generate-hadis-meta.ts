...
type GeneratedHadisMeta = HadisArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/hadis");
const OUTPUT_FILE = path.resolve("src/lib/generated/hadisMeta.ts");
...
function extractMeta(source: string): HadisArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedHadisMeta[] = files.map((filePath) => {
  ...
  console.log("Generated metadata for " + articles.length + " hadis articles.");
}
