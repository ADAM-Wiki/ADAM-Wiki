...
type GeneratedhriscanstvoMeta = hriscanstvoArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/hriscanstvo");
const OUTPUT_FILE = path.resolve("src/lib/generated/hriscanstvoMeta.ts");
...
function extractMeta(source: string): hriscanstvoArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedhriscanstvoMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " hriscanstvo articles.",
  );
}
