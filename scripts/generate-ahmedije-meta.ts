...
type GeneratedAhmedijeMeta = AhmedijeArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/ahmedije");
const OUTPUT_FILE = path.resolve("src/lib/generated/ahmedijeMeta.ts");
...
function extractMeta(source: string): AhmedijeArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedAhmedijeMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " ahmedije articles.",
  );
}
