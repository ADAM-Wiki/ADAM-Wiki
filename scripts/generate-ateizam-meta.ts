...
type GeneratedAteizamMeta = AteizamArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/ateizam");
const OUTPUT_FILE = path.resolve("src/lib/generated/ateizamMeta.ts");
...
function extractMeta(source: string): AteizamArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedAteizamMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " ateizam articles.",
  );
}
