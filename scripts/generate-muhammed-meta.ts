...
type GeneratedMuhammedMeta = MuhammedArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/muhammed");
const OUTPUT_FILE = path.resolve("src/lib/generated/muhammedMeta.ts");
...
function extractMeta(source: string): MuhammedArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedMuhammedMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " muhammed articles.",
  );
}
