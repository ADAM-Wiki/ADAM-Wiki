...
type GeneratedHinduizamMeta = HinduizamArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/hinduizam");
const OUTPUT_FILE = path.resolve("src/lib/generated/hinduizamMeta.ts");
...
function extractMeta(source: string): HinduizamArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedHinduizamMeta[] = files.map((filePath) => {
  ...
  console.log(
    "Generated metadata for " + articles.length + " hinduizam articles.",
  );
}
