...
type GeneratedNaukaMeta = NaukaArticleMeta & {
  wordCount: number;
...
const CONTENT_DIR = path.resolve("src/content/articles/nauka");
const OUTPUT_FILE = path.resolve("src/lib/generated/naukaMeta.ts");
...
function extractMeta(source: string): NaukaArticleMeta {
...
function generate(): void {
  ...
  const articles: GeneratedNaukaMeta[] = files.map((filePath) => {
  ...
  console.log("Generated metadata for " + articles.length + " nauka articles.");
}
